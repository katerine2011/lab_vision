...
%% %filtros
Im1 = rgb2gray(imread('4.2.01.tiff'));
%Im2 = imread('4.2.03.tiff');
...
J12 = imnoise(I,'poisson');
J13= imnoise(I,'salt & pepper', 0.09);
J1 = imnoise(I,'speckle', 0.02);
...
figure,
subplot(3,3,1),subimage(Im2),title('original')
...
%%
%template maching
image = imread('motion05.512.tiff');
[train,square] = imcrop(I0); %Extrae una parte de la imagen para ser tomada como template

c=normxcorr2(train,image);    % c is correlation from -1 to 1
[sx,sy] = size(image);      %tamaño de la imagen
d=floor(size(train)/2);     % entrga la parte entera más cercana para el valor del template
dx=d(1);                    % Tamaño en x
dy=d(2);                    %tamaño en y
%Remove borders from c2 to make it the same size as image
c2=c(dx+1:sx+dx,dy+1:sy+dy); % construye la matriz de correlación con los mismos valores de la imagen original
%lets accentuate it more
c3=c2.^3; % acentua la correlación
